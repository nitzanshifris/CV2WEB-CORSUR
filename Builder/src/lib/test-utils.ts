...
class TestRunner {
  ...
  addTest(name: string, fn: () => void | Promise<void>, options: TestOptions = {}): void {
        ...
        if (result instanceof Promise) {
          await Promise.race([
            ...
            new Promise((_, reject) =>
              setTimeout(() => reject(new Error('Test timeout')), test.options.timeout || 5000)
            ),
          ]);
        }
      } catch (e) {
  ...
  return {
    ...
    toHaveLength(expected: number): void {
      if (Array.isArray(actual)) {
        ...
        if (actual.length !== expected) {
          throw new Error(`Expected string to have length ${expected}, but got ${actual.length}`);
        }
      } else {
...
export function timeout(name: string, fn: () => void | Promise<void>, ms: number): void {
  testRunner.addTest(name, fn, { timeout: ms });
}

export function retry(name: string, fn: () => void | Promise<void>, count: number): void {
